module.exports = function( grunt ) {
    ...
    grunt.registerTask('unit-test', 'run the testacular test driver', function () {
        ...
        var done = this.async();
        require('child_process').exec('testacular start test/config/testacular.conf.js --single-run', function (err, stdout) {
            grunt.log.write(stdout);
            done(err);
        });
    });

    grunt.registerTask('e2e-test', 'run the testacular test driver', function () {

        var done = this.async();
        require('child_process').exec('testacular start test/config/testacular-e2e.conf.js --single-run', function (err, stdout) {
            grunt.log.write(stdout);
            done(err);
        });
    });
